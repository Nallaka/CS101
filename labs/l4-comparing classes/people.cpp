...
public:
	...
	Person(string f, string l, int c) : first(f), last(l), cwid(c) {
	...
	bool compare(Person &rhs, Person::CompareType t) {
		Person &lhs = *this;
		switch (t) {
			case CompareType::LessCwid:
				return lhs.getCwid() < rhs.getCwid();
			case CompareType::LessLastFirstCwid:
				if (lhs.getLast() < rhs.getLast()) return true;
				if (lhs.getLast() > rhs.getLast()) return false;
				if (lhs.getFirst() < rhs.getFirst()) return true;
				if (lhs.getFirst() > rhs.getFirst()) return false;
				return lhs.getCwid() < rhs.getCwid();
		}
	}
};
